namespace UnityTestTools.Assertions.Editor
{
    ...
    public class AssertionComponentEditor : Editor
    {
        ...
        {
            this.m_ComparerDropDown.convertForButtonLabel = type => type.Name;
            this.m_ComparerDropDown.convertForGUIContent = type => type.Name;
            this.m_ComparerDropDown.ignoreConvertForGUIContent = types => false;
            this.m_ComparerDropDown.tooltip = "Comparer that will be used to compare values and determine the result of assertion.";
        }

        public override void OnInspectorGUI()
        {
            var script = (AssertionComponent)this.target;
            EditorGUILayout.BeginHorizontal();
            var obj = this.DrawComparerSelection(script);
            ...
            {
                this.DrawOptionsForAfterPeriodOfTime(script);
            }

            if (script.IsCheckMethodSelected(CheckMethod.Update))
            {
                this.DrawOptionsForOnUpdate(script);
            }
            ...
            if (obj)
            {
                ...
                this.m_ThisPathSelector.Draw(script.Action.go, script.Action,
                ...
                this.DrawCustomFields(script);

                EditorGUILayout.Space();

                if (script.Action is ComparerBase)
                {
                    this.DrawCompareToType(script.Action as ComparerBase);
                }
            }
            if(GUI.changed)
        ...
        {
            EditorGUILayout.Space();
            script.checkAfterTime = EditorGUILayout.FloatField(this.m_GUICheckAfterTimeGuiContent,
                                                               script.checkAfterTime);
            if (script.checkAfterTime < 0)
                script.checkAfterTime = 0;
            script.repeatCheckTime = EditorGUILayout.Toggle(this.m_GUIRepeatCheckTimeGuiContent,
                                                            script.repeatCheckTime);
            if (script.repeatCheckTime)
            {
                script.repeatEveryTime = EditorGUILayout.FloatField(this.m_GUIRepeatEveryTimeGuiContent,
        ...
        {
            EditorGUILayout.Space();
            script.checkAfterFrames = EditorGUILayout.IntField(this.m_GUICheckAfterFramesGuiContent,
                                                               script.checkAfterFrames);
            if (script.checkAfterFrames < 1)
                script.checkAfterFrames = 1;
            script.repeatCheckFrame = EditorGUILayout.Toggle(this.m_GUIRepeatCheckFrameGuiContent,
                                                             script.repeatCheckFrame);
            if (script.repeatCheckFrame)
            {
                script.repeatEveryFrame = EditorGUILayout.IntField(this.m_GUIRepeatEveryTimeGuiContent,
        ...
        private void DrawCompareToType(ComparerBase comparer)
        {
            ...
            if (comparer.compareToType == ComparerBase.CompareToType.CompareToConstantValue)
            {
                try
                {
                    this.DrawConstCompareField(comparer);
                }
            ...
            {
                this.DrawObjectCompareField(comparer);
            }
        }

        private void DrawObjectCompareField(ComparerBase comparer)
        {
            this.m_OtherPathSelector.Draw(comparer.other, comparer,
        ...
        private bool DrawComparerSelection(AssertionComponent script)
        {
            ...
            this.m_ComparerDropDown.Draw(script.Action.GetType(), allComparers,
                                    type =>
                                    {
