namespace Commands
{
    ...
    public class NetworkCommandData
    {
        ...
        public static NetworkCommandData Parse(string command)
        {
            ...
            var textBeginIndex = nameLength.ToString().Length + optionsCount.ToString().Length + 2;
            textBeginIndex += optionsNamesLengthValuesLength.Sum(nlvl => nlvl.Key.ToString().Length + nlvl.Value.ToString().Length + 2);
            var text = command.Substring(textBeginIndex);
